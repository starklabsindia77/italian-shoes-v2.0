...
const FALLBACK: SizeItem[] = [
  { id: "size_us8", sizeId: "us-8", name: "US 8", region: "US", value: 8, euEquivalent: "EU 41", ukEquivalent: "UK 7", isActive: true, sortOrder: 80 },
  { id: "size_us9", sizeId: "us-9", name: "US 9", region: "US", value: 9, euEquivalent: "EU 42", ukEquivalent: "UK 8", isActive: true, sortOrder: 90 },
];
...
export default function SizesListPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/sizes?limit=300${query ? `&q=${encodeURIComponent(query)}` : ""}`, { cache: "no-store" });
  ...
  const sorted = React.useMemo(() => {
    const copy = [...items];
    copy.sort((a, b) => (sortAsc ? a.sortOrder - b.sortOrder : b.sortOrder - a.sortOrder));
    return copy;
  }, [items, sortAsc]);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage global size list used by all products.</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={load}><RefreshCcw className="mr-2 size-4" />Refresh</Button>
          <Button asChild><Link href="/sizes/new"><Plus className="mr-2 size-4" />New Size</Link></Button>
        </div>
      </div>
              ...
              <TableBody>
                {sorted.map((s) => (
                  <TableRow key={s.id}>
                    ...
                    <TableCell className="text-muted-foreground">{s.euEquivalent ?? "—"}</TableCell>
                    <TableCell className="text-muted-foreground">{s.ukEquivalent ?? "—"}</TableCell>
