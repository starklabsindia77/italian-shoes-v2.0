...
React.useEffect(() => {
 ...
 // only changes when materialId changes
  ...
  const add = async () => {
    ...
    setName("");
    setImageUrl(""); // keep hex/active as-is for quick adding
  };

  const toggleActive = async (row: MaterialColor, next: boolean) => {
    setItems((list) =>
      list.map((c) => (c.id === row.id ? { ...c, isActive: next } : c))
    );
    try {
  ...
  const remove = async (row: MaterialColor) => {
    const run = async () => {
      const res = await fetch(`/api/materials/${materialId}/colors/${row.id}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error(await res.text());
      return res.json();
  ...
  return (
    <Card className="rounded-2xl">
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Add and manage available colors for this material.
        </CardDescription>
      </CardHeader>
          ...
          <div className="grid gap-2">
            ...
            <div className="flex items-center justify-between rounded-md border p-2">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <PaintBucket className="size-4" /> Usable
              </div>
