...
export default function MaterialEditPage() {
  ...
  const createColor = async (payload: Omit<MaterialColor, "id" | "position">) => {
    ...
    const p = run();
    toast.promise(p, { loading: "Adding color…", success: "Color added", error: "Failed to add" });
    try {
  ...
  const updateColor = async (c: MaterialColor, patch: Partial<MaterialColor>) => {
    // optimistic
    setColors((prev) => prev.map((x) => (x.id === c.id ? { ...x, ...patch } : x)));
    const run = async () => {
  ...
  const deleteColor = async (c: MaterialColor) => {
    const run = async () => {
      const res = await fetch(`/api/materials/${id}/colors/${c.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(await res.text());
      return res.json();
    };
    const p = run();
        ...
        <TabsContent value="overview" className="mt-4">
          <Card className="rounded-2xl">
            <CardHeader className="pb-3">
              ...
              <CardDescription>Update material family metadata.</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-6 md:grid-cols-2">
              ...
              <Field label="Category (slug)">
                <Input value={material.category} onChange={(e) => setMaterial({ ...material, category: e.target.value })} />
              </Field>
              <div className="md:col-span-2">
                <Field label="Description">
                  <Textarea rows={6} value={material.description ?? ""} onChange={(e) => setMaterial({ ...material, description: e.target.value })} />
                </Field>
              </div>
              <div className="md:col-span-2 flex items-center justify-between rounded-lg border p-3">
                ...
                <Switch checked={material.isActive} onCheckedChange={(v) => setMaterial({ ...material, isActive: v })} />
              </div>
            </CardContent>
          </Card>
        </TabsContent>
  ...
  return (
    <Card className="rounded-2xl">
      <CardHeader className="pb-3">
        ...
        <CardDescription>Add and manage color values for this material.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="grid gap-3 md:grid-cols-5">
          ...
          <Field label="Type (group)">
            <Input placeholder="e.g., White, Black" value={type} onChange={(e) => setType(e.target.value)} />
          </Field>
          <Field label="Hex code">
            <Input placeholder="#000000" value={hex} onChange={(e) => setHex(e.target.value)} />
          </Field>
            ...
            <TableBody>
              {list.map((c) => (
                <TableRow key={c.id}>
                  ...
                  <TableCell>
                    {c.isActive ? <Badge>Active</Badge> : <Badge variant="secondary">Disabled</Badge>}
                  </TableCell>
                  <TableCell className="flex justify-end gap-2">
                    <Button size="sm" variant="outline" onClick={() => onToggle(c, !c.isActive)}>
                      {c.isActive ? <ToggleLeft className="mr-2 size-4" /> : <ToggleRight className="mr-2 size-4" />}
