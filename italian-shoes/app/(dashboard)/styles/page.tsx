...
export default function StylesListPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/styles?limit=100${query ? `&q=${encodeURIComponent(query)}` : ""}`, { cache: "no-store" });
  ...
  const toggleActive = async (s: StyleItem) => {
    ...
    try {
      const run = async () => {
        const res = await fetch(`/api/styles/${s.id}`, { method: "PUT", body: JSON.stringify({ isActive: !s.isActive }) });
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage shoe styles and their 3D configs.</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={load}><RefreshCcw className="mr-2 size-4" />Refresh</Button>
          <Button asChild><Link href="/styles/new"><Plus className="mr-2 size-4" />New Style</Link></Button>
        </div>
      </div>
