    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage customizable shoe panels.</p>
        </div>
      ...
      <Card className="rounded-2xl">
        <CardHeader className="pb-3">
          ...
          <CardDescription>Search, toggle status, edit, and delete.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Input placeholder="Search panels…" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => { if (e.key === "Enter") load(); }} />
            <Button onClick={load}>Search</Button>
            <Button variant="outline" onClick={() => setSortAsc(v => !v)}><ArrowUpDown className="mr-2 size-4" />{sortAsc ? "Sort Desc" : "Sort Asc"}</Button>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {sorted.map(p => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="flex justify-end gap-2">
                      ...
                      <Button size="sm" variant="destructive" onClick={() => deletePanel(p)}>
                ...
                {items.length === 0 && !loading && (
                  <TableRow>
                    <TableCell colSpan={6} className="py-6 text-center text-sm text-muted-foreground">No panels found.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
